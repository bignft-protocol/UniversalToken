...
- Or get the MNEMONIC generated by hardhat with the following command:

```
$ yarn hardhat node
```

The second option is recommended for development purposes since the wallets associated to the MNEMONIC will be pre-loaded with ETH for tests on hardhat.

#### 2.Load the wallet associated to the MNEMONIC with ether

If you've used hardhat to generate your MNEMONIC and you only want to perform tests on hardhat, you have nothing to do. The accounts are already loaded with 100 ETH.

For all other networks than hardhat, you have to send ether to the accounts associated to the MNEMONIC:
...
```bash
yarn task --network bscTestnet tasks/deployments/deploy-ERC1400.ts --input '{"name":"ERC1400Token","symbol":"DAU"}'

# contract-arguments.js generated from deploy task

yarn verify --network bscTestnet --constructor-args contract-arguments.js 0xDef49df8Ecad7b668D38A5Bde34a7F909Ac5caa8
```
