    ...
    IHoldableERC1400TokenExtension
{
    ...
    bytes4 internal constant ERC20_TRANSFER_ID =
        bytes4(keccak256("transfer(address,uint256)"));
    bytes4 internal constant ERC20_TRANSFERFROM_ID =
        bytes4(keccak256("transferFrom(address,address,uint256)"));

    bytes32 internal constant ZERO_ID = 0x00000000000000000000000000000000;
    ...
    function registerTokenSetup(
        ...
        _granularityByPartitionActivated[
            token
    ...
    function canValidate(
        IERC1400TokensValidator.ValidateData calldata data // Comments to avoid compilation warnings for unused variables.
    ) external view override returns (bool) {
        (bool canValidateToken, , ) = _canValidateCertificateToken(
            data.token,
            data.payload,
            data.operator,
            data.operatorData.length != 0 ? data.operatorData : data.data
        );
        ...
        canValidateToken =
            canValidateToken &&
            _canValidateGranularToken(data.token, data.partition, data.value);
    ...
    function tokensToValidate(
        ...
        {
            require(
                _canValidateGranularToken(msg.sender, partition, value),
                "50"
        ...
        {
            (, bytes32 holdId) = _retrieveHoldHashId(
                msg.sender,
                partition,
                operator,
                from,
    ...
    function _increaseHeldBalance(
        ...
        _heldBalanceByPartition[token][executableHold.sender][
            executableHold.partition
        ] = _heldBalanceByPartition[token][executableHold.sender][
            executableHold.partition
        ].add(executableHold.value);
        _totalHeldBalanceByPartition[token][
            executableHold.partition
        ] = _totalHeldBalanceByPartition[token][executableHold.partition].add(
            executableHold.value
        );
    }
    ...
    function _decreaseHeldBalance(
        ...
        _totalHeldBalanceByPartition[token][
            executableHold.partition
        ] = _totalHeldBalanceByPartition[token][executableHold.partition].sub(
            value
        );
    }
     ...
     */
    function _holdCanBeExecuted(Hold storage executableHold, uint value)
        internal
        view
        returns (bool)
    {
        ...
        returns (bool)
    {
        ...
        if (
            functionSig == ERC20_TRANSFER_ID ||
            functionSig == ERC20_TRANSFERFROM_ID
        ) {
            return false;
        } else {
            return true;
    ...
    function _checkNonceBasedCertificate(
        ...
        if (v == 27 || v == 28) {
            ...
            for (uint i = 0; i < payloadWithCertificate.length.sub(160); i++) {
    ...
    function _checkSaltBasedCertificate(
        ...
        if (v == 27 || v == 28) {
            ...
            for (uint i = 0; i < payloadWithCertificate.length.sub(192); i++) {
                // replace 4 bytes corresponding to function selector
                payloadWithoutCertificate[i] = payloadWithCertificate[i];
            }
