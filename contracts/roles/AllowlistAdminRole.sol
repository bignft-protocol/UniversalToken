...
abstract contract AllowlistAdminRole {
    ...
    function isAllowlistAdmin(address token, address account)
        public
        view
        returns (bool)
    {
    ...
    function addAllowlistAdmin(address token, address account)
        public
        onlyAllowlistAdmin(token)
    {
        _addAllowlistAdmin(token, account);
    }

    function removeAllowlistAdmin(address token, address account)
        public
        onlyAllowlistAdmin(token)
    {
