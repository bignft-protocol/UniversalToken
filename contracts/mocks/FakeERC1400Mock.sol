...
abstract contract ExtensionMock {
    ...
    function addAllowlistAdmin(address token, address account) external virtual;

    function addBlocklistAdmin(address token, address account) external virtual;

    function addPauser(address token, address account) external virtual;
}
...
contract FakeERC1400Mock is ERC1400 {
        ...
        ERC1400(name, symbol, granularity, controllers, defaultPartitions)
    {
        if (extension != address(0)) {
            ...
            ExtensionMock(extension).addAllowlistAdmin(
                address(this),
                mockAddress
            );
            ExtensionMock(extension).addBlocklistAdmin(
                address(this),
                mockAddress
            );
