...
contract BatchTokenIssuer is ERC1820Implementer {
    string internal constant BATCH_ISSUER = "BatchTokenIssuer";
    ...
    modifier onlyTokenMinter(address token) {
        require(
            IMinterRole(token).isMinter(msg.sender),
    ...
    function batchIssueByPartition(
        ...
        require(
            partitions.length == values.length,
            "partitions and values arrays have different lengths"
        );
