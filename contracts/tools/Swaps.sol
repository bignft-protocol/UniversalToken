    ...
    ERC1820Implementer
{
    ...
    bytes32 internal constant TRADE_PROPOSAL_FLAG =
        0xcccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccc;
    bytes32 internal constant TRADE_ACCEPTANCE_FLAG =
        0xdddddddddddddddddddddddddddddddddddddddddddddddddddddddddddddddd;

    bytes32 internal constant BYPASS_ACTION_FLAG =
        0xaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaa;

    bytes32 internal constant ALL_PARTITIONS =
    ...
    */
    ...
    function tokensReceived(
        ...
        if (flag == TRADE_PROPOSAL_FLAG) {
            ...
            UserTradeData memory _tradeData1 = UserTradeData(
                msg.sender,
                value,
                partition,
                Standard.ERC1400,
            ...
            UserTradeData memory selectedUserTradeData = (from == trade.holder1)
                ? trade.userTradeData1
                : trade.userTradeData2;
            require(
                msg.sender == selectedUserTradeData.tokenAddress,
                "Wrong token sent"
            );
            require(
                partition == selectedUserTradeData.tokenId,
                "Tokens of the wrong partition sent"
            );
            require(
                Standard.ERC1400 == selectedUserTradeData.tokenStandard,
    ...
    function requestTrade(
        TradeRequestInput calldata inputData,
        bytes32 preimage
    ) external payable {
        _requestTrade(
            inputData.holder1,
            ...
            UserTradeData(
                inputData.tokenAddress1,
                inputData.tokenValue1,
                inputData.tokenId1,
                inputData.tokenStandard1,
                false,
                false,
                inputData.tradeType1
            ),
            UserTradeData(
                inputData.tokenAddress2,
                inputData.tokenValue2,
                inputData.tokenId2,
                inputData.tokenStandard2,
                false,
                false,
                inputData.tradeType2
            )
        );

        if (
            msg.sender == inputData.holder1 || msg.sender == inputData.holder2
        ) {
    ...
    function _requestTrade(
        ...
        if (userTradeData1.tokenStandard == Standard.ETH) {
            require(
                userTradeData1.tradeType == TradeType.Escrow,
        ...
        if (userTradeData2.tokenStandard == Standard.ETH) {
            require(
                userTradeData2.tradeType == TradeType.Escrow,
                "Ether trades need to be of type Escrow"
            );
        }
        ...
        if (userTradeData1.tradeType == TradeType.Hold) {
            require(
                userTradeData1.tokenStandard == Standard.ERC20 ||
                    userTradeData1.tokenStandard == Standard.ERC1400,
                "Invalid token standard for hold trade type"
            );
            require(
                userTradeData1.tokenId != bytes32(0),
                "No holdId specified"
            );
        }
        ...
        if (userTradeData2.tradeType == TradeType.Hold) {
            require(
                userTradeData2.tokenStandard == Standard.ERC20 ||
                    userTradeData2.tokenStandard == Standard.ERC1400,
                "Invalid token standard for hold trade type"
            );
            require(
                userTradeData2.tokenId != bytes32(0),
                "No holdId specified"
            );
        }
    ...
    function _acceptTrade(
        ...
        UserTradeData memory selectedUserTradeData = (sender == trade.holder1)
            ? trade.userTradeData1
            : trade.userTradeData2;

        require(
            !selectedUserTradeData.accepted,
            "Trade already accepted by the holder"
        );

        if (selectedUserTradeData.tradeType == TradeType.Escrow) {
            if (selectedUserTradeData.tokenStandard == Standard.ETH) {
                require(
                    ethValue == selectedUserTradeData.tokenValue,
                    "Amount of ETH is not correct"
                );
            } else if (selectedUserTradeData.tokenStandard == Standard.ERC20) {
                IERC20(selectedUserTradeData.tokenAddress).transferFrom(
                    sender,
                    address(this),
                    selectedUserTradeData.tokenValue
                );
            } else if (selectedUserTradeData.tokenStandard == Standard.ERC721) {
                IERC721(selectedUserTradeData.tokenAddress).transferFrom(
                    sender,
                    address(this),
                    uint256(selectedUserTradeData.tokenId)
                );
            } else if (
                (selectedUserTradeData.tokenStandard == Standard.ERC1400) &&
                erc1400TokenValue == 0
            ) {
                IERC1400(selectedUserTradeData.tokenAddress)
                    .operatorTransferByPartition(
                        selectedUserTradeData.tokenId,
                        sender,
                        address(this),
                        selectedUserTradeData.tokenValue,
                ...
                require(
                    erc1400TokenValue == selectedUserTradeData.tokenValue,
                    "Amount of ERC1400 tokens is not correct"
                );
            }
        } else if (selectedUserTradeData.tradeType == TradeType.Hold) {
            require(
                _holdExists(sender, recipientHolder, selectedUserTradeData),
                "Hold needs to be provided in token smart contract first"
            );
        } else {
            // trade.tradeType == TradeType.Allowance
            require(
                _allowanceIsProvided(sender, selectedUserTradeData),
        ...
        if (
            settlementDatePassed &&
            trade.executer == address(0) &&
            trade.userTradeData1.tradeType == TradeType.Hold &&
            trade.userTradeData2.tradeType == TradeType.Hold &&
            tradeApproved
        ) {
            //we know selectedUserTradeData has a hold that exists, so check the other one
            UserTradeData memory otherUserTradeData = (sender == trade.holder1)
                ? trade.userTradeData2
                : trade.userTradeData1;
        ...
        if (
            trade.executer == address(0) &&
            getTradeAcceptanceStatus(index) &&
            tradeApproved &&
            settlementDatePassed
        ) {
        ...
        returns (bool)
    {
        ...
        if (trade.state == State.Pending) {
            if (
                trade.userTradeData1.tradeType == TradeType.Allowance &&
                !_allowanceIsProvided(trade.holder1, trade.userTradeData1)
            ) {
                return false;
            }
            if (
                trade.userTradeData2.tradeType == TradeType.Allowance &&
                !_allowanceIsProvided(trade.holder2, trade.userTradeData2)
            ) {
                return false;
            }
        }
     ...
     */
    function _allowanceIsProvided(
        address sender,
        UserTradeData memory userTradeData
        ...
        if (tokenStandard == Standard.ERC20) {
            return (IERC20(tokenAddress).allowance(sender, address(this)) >=
                tokenValue);
        } else if (tokenStandard == Standard.ERC721) {
            return (IERC721(tokenAddress).getApproved(uint256(tokenId)) ==
                address(this));
        } else if (tokenStandard == Standard.ERC1400) {
            return (IERC1400(tokenAddress).allowanceByPartition(
                tokenId,
                sender,
                address(this)
            ) >= tokenValue);
     ...
     */
    function approveTradeWithPreimage(
        uint256 index,
        bool approved,
        bytes32 preimage
    ) public {
        ...
        require(
            _isTokenController[trade.userTradeData1.tokenAddress][msg.sender] ||
                _isTokenController[trade.userTradeData2.tokenAddress][
                    msg.sender
                ],
    ...
    function getTradeApprovalStatus(uint256 index) public view returns (bool) {
        ...
        if (
            _tokenControllers[trade.userTradeData1.tokenAddress].length != 0 &&
            !trade.userTradeData1.approved
        ) {
            return false;
        }

        if (
            _tokenControllers[trade.userTradeData2.tokenAddress].length != 0 &&
            !trade.userTradeData2.approved
        ) {
            return false;
        }
    ...
    function executeTradeWithPreimage(uint256 index, bytes32 preimage) public {
        ...
        require(
            block.timestamp >= trade.settlementDate,
            "Trade can only be executed on or after settlement date"
        );
    ...
    function _transferUsersTokens(
        ...
        UserTradeData memory senderUserTradeData = (holder == Holder.Holder1)
            ? trade.userTradeData1
            : trade.userTradeData2;
    ...
    function _executeHoldOnUsersTokens(
        uint256 index,
        Holder holder,
        uint256,
        bool,
        ...
        require(
            tokenStandard == Standard.ERC20 ||
                tokenStandard == Standard.ERC1400,
            "Token standard must be holdable"
        );

        require(
            _holdExists(sender, recipient, senderUserTradeData),
            "Hold must exist"
        );
     ...
     */
    function _executeTransferOnUsersTokens(
        uint256 index,
        Holder holder,
        uint256 value,
        bool revertTransfer
        ...
        address recipient = (holder == Holder.Holder1)
            ? trade.holder2
            : trade.holder1;
        UserTradeData storage senderUserTradeData = (holder == Holder.Holder1)
            ? trade.userTradeData1
            : trade.userTradeData2;
        ...
        if (tokenStandard == Standard.ETH) {
            ...
            IERC1400(tokenAddress).operatorTransferByPartition(
                tokenId,
                currentHolder,
                recipient,
                value,
    ...
    function _executeHold(
        ...
        if (tokenStandard == Standard.ERC20) {
            ...
            uint256 holdValue;
            (, , , , holdValue, , , , ) = IHoldableERC1400TokenExtension(
                tokenExtension
        ...
        emit ExecutedHold(token, tokenHoldId, preimage, tokenRecipient);
    }

    function _holdExists(
        address sender,
        address recipient,
        UserTradeData memory userTradeData
    ) internal view returns (bool) {
        ...
        if (tokenStandard == Standard.ERC1400) {
            ...
            (
                ...
                ,
                holdStatus
            ) = IHoldableERC1400TokenExtension(tokenExtension).retrieveHoldData(
                    tokenAddress,
                    holdId
                );
            return
                holdStatus == HoldStatusCode.Ordered &&
                holdValue == userTradeData.tokenValue &&
                (holdSender == sender || holdSender == address(0)) &&
                holdRecipient == recipient &&
                (secretHash != bytes32(0) || notary == address(this));
        } else if (tokenStandard == Standard.ERC20) {
            ERC20HoldData memory data = IERC20HoldableToken(tokenAddress)
                .retrieveHoldData(holdId);
            return
                (data.sender == sender || data.sender == address(0)) &&
                data.recipient == recipient &&
                data.amount == userTradeData.tokenValue &&
                data.status == HoldStatusCode.Ordered &&
                (data.secretHash != bytes32(0) || data.notary == address(this));
        } else {
        ...
        returns (bool)
    {
        ...
        if (data.length == 320 && flag == TRADE_PROPOSAL_FLAG) {
            ...
            (data.length == 64 || data.length == 96) &&
            flag == TRADE_ACCEPTANCE_FLAG
        ) {
            return true;
    ...
    function _getTradeTokenData(bytes memory data)
        internal
        pure
        returns (UserTradeData memory tokenData)
    {
        address tokenAddress;
        uint256 tokenAmount;
        bytes32 tokenId;
    ...
    function setTokenControllers(
        ...
        for (uint i = 0; i < _tokenControllers[tokenAddress].length; i++) {
            _isTokenController[tokenAddress][
                _tokenControllers[tokenAddress][i]
            ] = false;
        }
        for (uint j = 0; j < operators.length; j++) {
        ...
        onlyPriceOracle(tokenAddress)
    {
        for (uint i = 0; i < _priceOracles[tokenAddress].length; i++) {
            _isPriceOracle[tokenAddress][
                _priceOracles[tokenAddress][i]
            ] = false;
        }
    ...
    function getTokenPrice(
        ...
        return
            _tokenUnitPricesByPartition[tokenAddress1][tokenAddress2][tokenId1][
                tokenId2
            ];
    }
    ...
    function getPrice(uint256 index) public view returns (uint256) {
        ...
        if (
            ...
            if (
                _tokenUnitPricesByPartition[tokenAddress1][tokenAddress2][
                    tokenId1
                ][tokenId2] != 0
            ) {
                return
                    tokenValue1 *
                    (
                        _tokenUnitPricesByPartition[tokenAddress1][
                            tokenAddress2
                        ][tokenId1][tokenId2]
                    );
            } else if (
                _tokenUnitPricesByPartition[tokenAddress2][tokenAddress1][
                    tokenId2
                ][tokenId1] != 0
            ) {
                return
                    tokenValue1 /
                    (
                        _tokenUnitPricesByPartition[tokenAddress2][
                            tokenAddress1
                        ][tokenId2][tokenId1]
                    );
            } else if (
