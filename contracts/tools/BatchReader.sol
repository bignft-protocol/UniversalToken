...
abstract contract IERC1400TokensValidatorExtended is IExtensionTypes {
    ...
    function isAllowlisted(address token, address account)
        public
        view
        virtual
        returns (bool);

    function isBlocklisted(address token, address account)
        public
        view
        virtual
        returns (bool);
...
contract BatchReader is IExtensionTypes, ERC1820Client, ERC1820Implementer {
     ...
     */
    function batchTokenSuppliesInfos(address[] calldata tokens)
        external
        view
        returns (
            uint256[] memory,
        ...
        )
    {
        ...
        (
            uint256[] memory totalPartitionsLengths,
            bytes32[] memory batchTotalPartitions,
            uint256[] memory batchPartitionSupplies
        ) = batchTotalPartitions(tokens);

        (
            uint256[] memory defaultPartitionsLengths,
            bytes32[] memory batchDefaultPartitions
        ) = batchDefaultPartitions(tokens);

        return (
            batchTotalSupplies,
            totalPartitionsLengths,
            batchTotalPartitions,
            batchPartitionSupplies,
            defaultPartitionsLengths,
     ...
     */
    function batchTokenRolesInfos(address[] calldata tokens)
        external
        view
        returns (
            address[] memory,
    ...
    {
        (
            uint256[] memory batchExtensionControllersLength,
            address[] memory batchExtensionControllers
        ) = batchExtensionControllers(tokens);

        (
            uint256[] memory batchControllersLength,
            address[] memory batchControllers
        ) = batchControllers(tokens);
        ...
        return (
            batchOwners,
            batchControllersLength,
            batchControllers,
            batchExtensionControllersLength,
     ...
     */
    function batchControllers(address[] memory tokens)
        public
        view
        returns (uint256[] memory, address[] memory)
    {
        ...
        for (uint256 i = 0; i < tokens.length; i++) {
            address[] memory controllers = IERC1400Extended(tokens[i])
                .controllers();
            batchControllersLength[i] = controllers.length;
            controllersLength = controllersLength.add(controllers.length);
        }

        address[] memory batchControllersResponse = new address[](
            controllersLength
        );

        uint256 counter = 0;
        for (uint256 j = 0; j < tokens.length; j++) {
            address[] memory controllers = IERC1400Extended(tokens[j])
                .controllers();
     ...
     */
    function batchExtensionControllers(address[] memory tokens)
        public
        view
        returns (uint256[] memory, address[] memory)
    {
        ...
        uint256[] memory batchExtensionControllersLength = new uint256[](
            tokens.length
        );
        uint256 extensionControllersLength = 0;

        for (uint256 i = 0; i < tokens.length; i++) {
            batchTokenExtension[i] = interfaceAddr(
                tokens[i],
            ...
            if (batchTokenExtension[i] != address(0)) {
                (
                    ...
                    ,
                    ,
                    address[] memory extensionControllers
                ) = IERC1400TokensValidatorExtended(batchTokenExtension[i])
                        .retrieveTokenSetup(tokens[i]);
                batchExtensionControllersLength[i] = extensionControllers
                    .length;
                extensionControllersLength = extensionControllersLength.add(
                    extensionControllers.length
                );
            } else {
        ...
        address[] memory batchExtensionControllersResponse = new address[](
            extensionControllersLength
        );
        ...
        for (uint256 j = 0; j < tokens.length; j++) {
            if (batchTokenExtension[j] != address(0)) {
                (
                    ...
                    ,
                    address[] memory extensionControllers
                ) = IERC1400TokensValidatorExtended(batchTokenExtension[j])
                        .retrieveTokenSetup(tokens[j]);

                for (uint256 k = 0; k < extensionControllers.length; k++) {
                    batchExtensionControllersResponse[
                        counter
                    ] = extensionControllers[k];
                    counter++;
                }
            }
        }
     ...
     */
    function batchTokenExtensionSetup(address[] calldata tokens)
        external
        view
        returns (
            address[] memory,
            CertificateValidation[] memory,
            bool[] memory,
            bool[] memory,
            bool[] memory,
            bool[] memory
    ...
    {
        (
            address[] memory batchTokenExtension,
            CertificateValidation[] memory batchCertificateActivated,
            bool[] memory batchAllowlistActivated,
            bool[] memory batchBlocklistActivated
        ) = batchTokenExtensionSetup1(tokens);

        (
            bool[] memory batchGranularityByPartitionActivated,
            bool[] memory batchHoldsActivated
        ) = batchTokenExtensionSetup2(tokens);
        return (
            batchTokenExtension,
            batchCertificateActivated,
            batchAllowlistActivated,
            batchBlocklistActivated,
            batchGranularityByPartitionActivated,
            batchHoldsActivated
        );
    }
     ...
     */
    function batchTokenExtensionSetup1(address[] memory tokens)
        public
        view
        returns (
            address[] memory,
        ...
        )
    {
        address[] memory batchTokenExtension = new address[](tokens.length);
        CertificateValidation[]
            memory batchCertificateActivated = new CertificateValidation[](
                tokens.length
            );
     ...
     */
    function batchTokenExtensionSetup2(address[] memory tokens)
        public
        view
        returns (bool[] memory, bool[] memory)
    {
        address[] memory batchTokenExtension = new address[](tokens.length);
        bool[] memory batchGranularityByPartitionActivated = new bool[](
            tokens.length
        );
        ...
        for (uint256 i = 0; i < tokens.length; i++) {
            ...
            if (batchTokenExtension[i] != address(0)) {
                ...
                batchGranularityByPartitionActivated[
                    i
                ] = granularityByPartitionActivated;
        ...
        )
    {
        (
            ,
            ,
            uint256[] memory batchSpendableBalancesOfByPartition
        ) = batchSpendableBalanceOfByPartition(tokens, tokenHolders);

        (
            uint256[] memory totalPartitionsLengths,
            bytes32[] memory batchTotalPartitions,
            uint256[] memory batchBalancesOfByPartition
        ) = batchBalanceOfByPartition(tokens, tokenHolders);
        ...
        return (
            ...
            totalPartitionsLengths,
            batchTotalPartitions,
            batchBalancesOfByPartition,
            batchSpendableBalancesOfByPartition
        );
    }
     ...
     */
    function batchEthBalance(address[] memory tokenHolders)
        public
        view
        returns (uint256[] memory)
    {
        uint256[] memory batchEthBalanceResponse = new uint256[](
            tokenHolders.length
        );
     ...
     */
    function batchERC721Balances(
        address[] calldata tokens,
        address[] calldata tokenHolders
    ) external view returns (uint256[] memory, uint256[][][] memory) {
        uint256[][][] memory batchBalanceOfResponse = new uint256[][][](
            tokens.length
        );

        for (uint256 j = 0; j < tokens.length; j++) {
            IERC721Enumerable token = IERC721Enumerable(tokens[j]);
            uint256[][] memory batchBalance = new uint256[][](
                tokenHolders.length
            );
    ...
    function batchBalanceOf(
        ...
        for (uint256 i = 0; i < tokenHolders.length; i++) {
            for (uint256 j = 0; j < tokens.length; j++) {
                batchBalanceOfResponse[i * tokens.length + j] = IERC20(
                    tokens[j]
     ...
     */
    function batchBalanceOfByPartition(
        address[] memory tokens,
        address[] memory tokenHolders
    )
        public
        ...
        )
    {
        ...
        uint256[] memory batchBalanceOfByPartitionResponse = new uint256[](
            tokenHolders.length * batchTotalPartitions.length
        );
        ...
        for (uint256 i = 0; i < tokenHolders.length; i++) {
            ...
            for (uint256 j = 0; j < tokens.length; j++) {
                for (uint256 k = 0; k < totalPartitionsLengths[j]; k++) {
                    batchBalanceOfByPartitionResponse[
                        i * batchTotalPartitions.length + counter
                    ] = IERC1400(tokens[j]).balanceOfByPartition(
                        batchTotalPartitions[counter],
                        tokenHolders[i]
                    );
                    counter++;
                }
     ...
     */
    function batchSpendableBalanceOfByPartition(
        address[] memory tokens,
        address[] memory tokenHolders
    )
        public
        ...
        )
    {
        ...
        uint256[]
            memory batchSpendableBalanceOfByPartitionResponse = new uint256[](
                tokenHolders.length * batchTotalPartitions.length
            );
        ...
        for (uint256 i = 0; i < tokenHolders.length; i++) {
            uint256 counter = 0;
            for (uint256 j = 0; j < tokens.length; j++) {
                address tokenExtension = interfaceAddr(
                    tokens[j],
                    ERC1400_TOKENS_VALIDATOR
                );
                ...
                for (uint256 k = 0; k < totalPartitionsLengths[j]; k++) {
                    if (tokenExtension != address(0)) {
                        batchSpendableBalanceOfByPartitionResponse[
                            i * batchTotalPartitions.length + counter
                        ] = IERC1400TokensValidatorExtended(tokenExtension)
     ...
     */
    function batchTotalPartitions(address[] memory tokens)
        public
        view
        returns (
            uint256[] memory,
    ...
    {
        uint256[] memory batchTotalPartitionsLength = new uint256[](
            tokens.length
        );
        uint256 totalPartitionsLength = 0;

        for (uint256 i = 0; i < tokens.length; i++) {
            bytes32[] memory totalPartitions = IERC1400Extended(tokens[i])
                .totalPartitions();
            batchTotalPartitionsLength[i] = totalPartitions.length;
            totalPartitionsLength = totalPartitionsLength.add(
                totalPartitions.length
            );
        }

        bytes32[] memory batchTotalPartitionsResponse = new bytes32[](
            totalPartitionsLength
        );
        uint256[] memory batchPartitionSupplies = new uint256[](
            totalPartitionsLength
        );

        uint256 counter = 0;
        for (uint256 j = 0; j < tokens.length; j++) {
            bytes32[] memory totalPartitions = IERC1400Extended(tokens[j])
                .totalPartitions();

            for (uint256 k = 0; k < totalPartitions.length; k++) {
                batchTotalPartitionsResponse[counter] = totalPartitions[k];
                batchPartitionSupplies[counter] = IERC1400Extended(tokens[j])
                    .totalSupplyByPartition(totalPartitions[k]);
                counter++;
            }
        }

        return (
            batchTotalPartitionsLength,
            batchTotalPartitionsResponse,
            batchPartitionSupplies
        );
    }
     ...
     */
    function batchDefaultPartitions(address[] memory tokens)
        public
        view
        returns (uint256[] memory, bytes32[] memory)
    {
        uint256[] memory batchDefaultPartitionsLength = new uint256[](
            tokens.length
        );
        uint256 defaultPartitionsLength = 0;

        for (uint256 i = 0; i < tokens.length; i++) {
            bytes32[] memory defaultPartitions = IERC1400Extended(tokens[i])
                .getDefaultPartitions();
            batchDefaultPartitionsLength[i] = defaultPartitions.length;
            defaultPartitionsLength = defaultPartitionsLength.add(
                defaultPartitions.length
            );
        }

        bytes32[] memory batchDefaultPartitionsResponse = new bytes32[](
            defaultPartitionsLength
        );

        uint256 counter = 0;
        for (uint256 j = 0; j < tokens.length; j++) {
            bytes32[] memory defaultPartitions = IERC1400Extended(tokens[j])
                .getDefaultPartitions();
     ...
     */
    function batchAllowlisted(
        address[] memory tokens,
        address[] memory tokenHolders
    ) public view returns (bool[] memory) {
        bool[] memory batchAllowlistedResponse = new bool[](
            tokenHolders.length * tokens.length
        );
        ...
        for (uint256 i = 0; i < tokenHolders.length; i++) {
            for (uint256 j = 0; j < tokens.length; j++) {
                ...
                if (tokenExtension != address(0)) {
                    batchAllowlistedResponse[
                        i * tokens.length + j
                    ] = IERC1400TokensValidatorExtended(tokenExtension)
                        .isAllowlisted(tokens[j], tokenHolders[i]);
                } else {
     ...
     */
    function batchBlocklisted(
        address[] memory tokens,
        address[] memory tokenHolders
    ) public view returns (bool[] memory) {
        bool[] memory batchBlocklistedResponse = new bool[](
            tokenHolders.length * tokens.length
        );
        ...
        for (uint256 i = 0; i < tokenHolders.length; i++) {
            for (uint256 j = 0; j < tokens.length; j++) {
                ...
                if (tokenExtension != address(0)) {
                    batchBlocklistedResponse[
                        i * tokens.length + j
                    ] = IERC1400TokensValidatorExtended(tokenExtension)
                        .isBlocklisted(tokens[j], tokenHolders[i]);
                } else {
