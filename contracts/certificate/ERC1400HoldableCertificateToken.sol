...
contract ERC1400HoldableCertificateToken is ERC1400, IExtensionTypes {
    ...
    {
        return
            ERC1400._canTransfer(
                _replaceFunctionSelector(
                    this.transferByPartition.selector,
                    msg.data
                ), // 0xf3d490db: 4 first bytes of keccak256(transferByPartition(bytes32,address,uint256,bytes))
                partition,
                msg.sender,
                msg.sender,
                to,
        ...
        return
            ERC1400._canTransfer(
                _replaceFunctionSelector(
                    ...
                    msg.data
                ), // 0x8c0dee9c: 4 first bytes of keccak256(operatorTransferByPartition(bytes32,address,address,uint256,bytes,bytes))
                partition,
                msg.sender,
                from,
                to,
     ...
     */
    function _replaceFunctionSelector(bytes4 functionSig, bytes memory payload)
        internal
        pure
